 /****************************************************
  *  wpparc PPA 3 - case series sem Savage.c         *
  *                                                  *
  *  WEAVER++/ARC application to case series study   *
  *  of the semantic variant of primary progressive  *
  *  aphasia (PPA) by Savage et al. (2014)           *
  *                                                  *
  *  Simulation of cases of semantic variant of PPA  *
  *                                                  *
...
Real data for semantic variant of PPA on single word tasks:
Savage, S. A., Piguet, O., & Hodges, J. R. (2014). Giving words new life: 
Generalization of word retraining outcomes in semantic dementia. Journal 
of Alzheimer's Disease, 40(2), 309–317.
...
/* Dummy data on controls (Savage et al., 2013) and data on cases of 
   semantic variant PPA (Savage et al., 2014) for single word tasks */

double REAL_DATA[N_ASSESSMENTs][N_TASKs] = {
	      /* Naming  Comprehension Repetition */
	/* 0 */ { 88.7,    97.0,    99.7 }, /* controls */
	/* 1 */ { 26.7,    56.7,    96.7 }, /* patients */
	/* 2 */ { 26.7,    56.7,    96.7 },
	/* 3 */ {  3.3,    43.3,    83.3 },
	/* 4 */ {  6.7,    40.0,    83.3 },
	/* 5 */ {  6.7,    50.0,    80.0 }
...
/* weight lesion */
 ...
 /* Semantic dementia: connections to, within, and from conceptual network*/
 
/* decay lesion */
 double DECAY_INCREASE_SEMANTIC_DEMENTIA;
 /* Semantic dementia: concepts */
 ...
 *****************************/
 ...
 for (i = 0; i < N_PHONEMEs; i++)
	  for (j = 0; j < N_PHONEMEs; j++)
		input_oP[i] += (iP_node_act[j] * PP_con[j][i]  );
 ...
 /* input activation for input phoneme nodes */
  for(i=0;i<N_PHONEMEs;i++)
     for(j=0;j<N_PHONEMEs;j++) 
	input_iP[i]+=( oP_node_act[j] * PP_con[j][i] );
 ...
 *********************/
 ...
 void print_heading()
 {
	 ...
	 printf("Simulating case series of the semantic variant\n");
	 printf("Empirical data on individual patients from Savage et al. (2014)\n");
